...
+ (instancetype)buttonWithType:(UIButtonType)buttonType {
    MSTabItem *item = [super buttonWithType:buttonType];
    [item setup];
    return item;
}
...
- (void)setup {
    ...
    _badgeStyle = MSTabItemBadgeStyleNumber;
    self.badge = 0;
}

/**
 *  覆盖父类的setHighlighted:方法，按下MSTabItem时，不高亮该item
 */
...
- (void)setBadgeStyle:(MSTabItemBadgeStyle)badgeStyle {
    _badgeStyle = badgeStyle;
    [self updateBadge];
}

- (void)updateBadge {
    if (self.badgeStyle == MSTabItemBadgeStyleNumber) {
        if (self.badge == 0) {
    ...
    } else if (self.badgeStyle == MSTabItemBadgeStyleDot) {
