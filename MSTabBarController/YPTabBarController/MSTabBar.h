...
- (BOOL)MS_tabBar:(MSTabBar *)tabBar willSelectItemAtIndex:(NSInteger)index;
- (void)MS_tabBar:(MSTabBar *)tabBar didSelectedItemAtIndex:(NSInteger)index;

@end

@interface MSTabBar : UIView <UIScrollViewDelegate>

/**
 *  TabItems，提供给MSTabBarController使用，一般不手动设置此属性
 */
@property (nonatomic, copy) NSArray <MSTabItem *> *items;
...
/**
 *  添加一个特殊的MSTabItem到tabBar上，此TabItem不包含在tabBar的items数组里
 *  主要用于有的项目需要在tabBar的中间放置一个单独的按钮，类似于新浪微博等。
 *  此方法仅适用于不可滚动类型的tabBar
 *
 *  @param item    MSTabItem对象
 *  @param index   将其放在此index的item后面
 *  @param handler 点击事件回调
 */
- (void)setSpecialItem:(MSTabItem *)item
    afterItemWithIndex:(NSInteger)index
            tapHandler:(void (^)(MSTabItem *item))handler;
