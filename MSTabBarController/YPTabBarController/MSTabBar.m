...
@property (nonatomic, strong) MSTabItem *specialItem;
@property (nonatomic, copy) void (^specialItemHandler)(MSTabItem *item);

// 选中背景
@property (nonatomic, strong) UIImageView *itemSelectedBgImageView;

// 选中背景相对于MSTabItem的insets
...
- (void)setItems:(NSArray *)items {
    ...
    // 初始化每一个item
    for (MSTabItem *item in self.items) {
...
- (void)setTitles:(NSArray *)titles {
    ...
    for (NSString *title in titles) {
        MSTabItem *item = [[MSTabItem alloc] init];
        item.title = title;
...
- (void)updateItemsFrame {
    ...
    if (self.scrollView) {
        ...
        for (NSInteger index = 0; index < self.items.count; index++) {
            MSTabItem *item = self.items[index];
            CGFloat width = 0;
            // item的宽度为一个固定值
            if (self.itemWidth > 0) {
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (_selectedItemIndex >= 0) {
        MSTabItem *oldSelectedItem = self.items[_selectedItemIndex];
    ...
    MSTabItem *newSelectedItem = self.items[selectedItemIndex];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(MS_tabBar:didSelectedItemAtIndex:)]) {
        [self.delegate MS_tabBar:self didSelectedItemAtIndex:selectedItemIndex];
    }
...
- (void)tabItemClicked:(MSTabItem *)item {
    if (self.selectedItemIndex == item.index) {
        return;
    }
    BOOL will = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(MS_tabBar:willSelectItemAtIndex:)]) {
        will = [self.delegate MS_tabBar:self willSelectItemAtIndex:item.index];
    }
    if (will) {
...
- (void)specialItemClicked:(MSTabItem *)item {
    if (self.specialItemHandler) {
        self.specialItemHandler(item);
    }
}

- (MSTabItem *)selectedItem {
...
- (void)setSpecialItem:(MSTabItem *)item afterItemWithIndex:(NSInteger)index tapHandler:(void (^)(MSTabItem *item))handler {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    MSTabItem *leftItem = self.items[leftIndex];
    MSTabItem *rightItem = nil;
...
- (void)updateSeperators {
    if (self.itemSeparatorColor) {
        ...
        [self.items enumerateObjectsUsingBlock:^(MSTabItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx > 0) {
